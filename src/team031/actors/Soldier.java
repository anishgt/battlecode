 ...
 */
public class Soldier implements Actor {
    private WayPoint to;
...
//            }
    ...
    public boolean act(boolean move, boolean attack) throws GameActionException {
        ...
        Debug.setIndicatorString(0, waiting + "," + hostile.length + "," + to.type + "," + to.to);
        ...
        boolean viperInfected = rc.getViperInfectedTurns() >= 1;

        // something in attack range
        if (hostileARS.length > 0 && !viperInfected) {
        ...
        if (attacking[Direction.OMNI.ordinal()] > 0 && !viperInfected) {
        ...
        if (hostileARS.length > 0) {
            // TODO improve this
            if (attack) {
                if (enemy.distanceSquaredTo(here) <= c.ars) {
        ...
        if (hostile.length > 0) {
            ...
            if (viperInfected) {
                // be more aggressive
                Direction dir = Mover.avoidInDir(canMove, turretCount, hereEnemy);
                if (dir != null && dir != Direction.OMNI) {
                    if (move) {
                        bugger.endBug();
                        rc.move(dir);
...
//                Debug.setIndicatorString(0, Arrays.toString(idle));
