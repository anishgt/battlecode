...
public class Archon extends Robot{
	...
	public static void archonCode() throws GameActionException {
		...
		if(gameState == ZOMBIE_APOCALYPSE && isLeader)
		{
			System.out.println("Leader moving ");
			underAttackActions();
			while(rc.isCoreReady()){
				
				while(rc.canMove(Direction.SOUTH))
				{
					rc.move(Direction.SOUTH);
				}
				while(rc.canMove(Direction.WEST))
				{
...
//					assembleY = targetY;
		...
		else if(gameState==STATE_EXPLORE){
			exploreStateActions();
	...
	public static void selctGameState() throws GameActionException{
		if(rc.getRobotCount()>32 && targetX!=-1 && targetY!=-1){
			gameState=STATE_ATTACK;
			rc.broadcastMessageSignal(CHANGE_STATE, STATE_ATTACK, INFINITY);
		}
	}
	
	public static void zombieStateActions() throws GameActionException{
		if(unitProduction()==1){
	...
	public static int unitProduction() throws GameActionException {
		if(rc.isCoreReady()){
			...
			else if(rc.getRoundNum()<=5 && gameState==STATE_EXPLORE){
				createBuildList(0);
				toBuild = buildList[rnd.nextInt(buildList.length)];
			}else{
				createBuildList(1);
	...
	private static void createBuildList(int stage){
		if (stage==0)
			...
			buildList = new RobotType[]{RobotType.VIPER,RobotType.VIPER,RobotType.VIPER,RobotType.VIPER,RobotType.VIPER,RobotType.VIPER};
		}
	}
